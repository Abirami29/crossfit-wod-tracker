...
def synthetic_score(workout, days_ago, total_days=90):
    """
    ...
    simulating a fitness gain over the 90-day window). For Open workouts
    (no bands), just picks a plausible-looking number in a generic range -
    there's no real reference to anchor to, so this is illustrative only.
    """
    ...
    if bands:
        ...
        if workout["lower_is_better"]:
            return round(base * (1 - improvement), 1)
        else:
            return round(base * (1 + improvement), 1)
...
while d <= today:
    if random.random() < 0.45:
        ...
        score = synthetic_score(workout, days_ago)
        sessions.append({
            "date": d.isoformat(),
